...
for x in range(3):
    ...
    img = cv2.imread('./Task6/data/Bowls/1.' + str(x+1) + str('.') + str('jpeg'),0)
    print(img.shape)
    edges = cv2.Canny(img,100,200)
    img = cv2.imwrite('./Task6/data/my_data/edge_detected/test/cups/1.' + str(x+1) + str('.') + str('jpeg'),edges)

# Visualizing grayscaled and edged images
img = cv2.imread('./Task6/data/my_data/edge_detected/test/cups/1.1.jpeg',0)
edges = cv2.Canny(img,100,200)
